...
public class CarDaoImpl implements CarDao {
	...
	public Car getCarDetail(int id) {
		return (Car) entityManager.unwrap(Session.class)
				.createCriteria(Car.class).add(Restrictions.eq("id", id))
				.setFetchMode("customer", FetchMode.JOIN)
				.setFetchMode("imageses", FetchMode.JOIN).uniqueResult();
	...
	public List<String> getCarTypeCode() {
		return entityManager.unwrap(Session.class)
				.createCriteria(Car.class)
				.setProjection(Projections.distinct(Projections.property("typeCode")))
				.list();
	}
